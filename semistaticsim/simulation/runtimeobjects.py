...
from semistaticsim.reconstruction.scenedata import _Hippo, HippoObject
from semistaticsim.simulation.spatialutils.proximity_spatial_funcs import isOnTop, isInside, isBeside, distance
from semistaticsim.utils.dict2xyztup import dict2xyztuple, xyztuple_precision
from semistaticsim.utils.git_diff import git_diff
...
class RuntimeObject(_Hippo):
    ...
    def fromdict(cls, dico):
        ...
        from semistaticsim.simulation.skillsandconditions.skills_abstract import SkillPortfolio
...
class RuntimeObjectContainer(_Hippo):
    ...
    def coerce_ai2thor_metadata_objects(cls, metadata_objects):
        ...
        for o in metadata_objects:
            ...
            skill_metadata = []
            NATURAL_LANGUAGE_MAPPING = {'receptacle': 'objects can be put down on this', 'toggleable': 'can be turned on/off', 'breakable': 'can be broken', 'dirtyable': 'can be considered dirty or clean', 'sliceable': 'can be sliced', 'openable': 'can be opened and closed', 'pickupable': 'can be picked up'}
            for k, v in NATURAL_LANGUAGE_MAPPING.items():
                if o[k]:
    ...
    def get_object_by_id_might_not_exist(self, object_id, sas=None):
        try:
            ...
            from semistaticsim.simulation.skillsandconditions.conditions import CONDITION_ObjectExists, maybe_raise_condition_exception
